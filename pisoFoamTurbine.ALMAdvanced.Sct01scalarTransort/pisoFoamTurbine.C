...
int main(int argc, char *argv[])
{
    ...
    while (runTime.loop())
    {
        ...
        (
            "Sc_t",
            dimless,
            transportProperties.lookup("Sc_t")
        );
        ...
        fvScalarMatrix sPhiEqn
            (
                fvm::ddt(sPhi)
              + fvm::div(phi, sPhi)
             ==
                fvm::laplacian(turbulence->nu() / Sc_t + turbulence->nut() / Sct, sPhi)
            );
        ...
        sPhi.max(0.0);
        sPhi.min(1.0);
        sPhi.correctBoundaryConditions();  // 更新边界条件
